        ...
        <div className="space-y-6 h-full flex flex-col">
            <div className="flex-1 space-y-5 overflow-y-auto px-1">
                ...
                {isRevision && managerNotes && (
                    <div className="p-4 bg-orange-50 border border-orange-200 rounded-xl animate-in fade-in slide-in-from-top-2">
                        <div className="flex gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-orange-700">{managerNotes}</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                <div className="space-y-4">
                    <div className="space-y-2">
                        <Label className="text-slate-700 font-semibold text-base">Tanggal Jurnal</Label>
                            ...
                            <>
                                <Button
                                    ...
                                    className={cn(
                                        ...
                                        isDateConflict && "border-red-300 bg-red-50 text-red-900",
                                        props.isDateLocked && "opacity-50 cursor-not-allowed bg-slate-100"
                                    )}
                                >
                    ...
                    {isDateConflict && (
                        <div className="p-3 bg-red-50 border border-red-100 rounded-xl relative overflow-hidden">
                            <div className="flex gap-3 relative z-10">
                                <AlertTriangle className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
                                <div>
                                    <p className="text-sm font-semibold text-red-800 mb-1">
                                        Sudah ada jurnal di tanggal ini
                                    </p>
                                    <p className="text-xs text-red-700 mb-2">
                                        Kamu hanya bisa membuat satu jurnal per hari. Apakah kamu ingin mengedit jurnal yang sudah ada?
                                    </p>
                                    {props.onRequestEdit && (
                                        <Button
                                            size="sm"
                                        ...
                                        >
                                            Edit Jurnal {format(date, "d MMM", { locale: id })}
                                        </Button>
                                    )}
                                </div>
                            </div>
                    ...
                    {!isToday && !isDateConflict && (
                        <div className="p-3 bg-amber-50 border border-amber-100 rounded-xl flex items-start gap-3">
                            <span className="text-lg">🗓️</span>
                            <div>
                                <p className="text-sm text-amber-800 font-medium">
                                    Kamu mengisi jurnal untuk tanggal: <span className="font-bold">{format(date, "dd MMMM yyyy", { locale: id })}</span>
                                </p>
                                <p className="text-xs text-amber-700 mt-1">
                                    Tidak masalah lupa sesekali 🙂. Silakan isi kegiatanmu di hari tersebut.
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    {isMobile ? (
                        <>
                            <button
                                ...
                                className="w-full flex items-center justify-between p-4 border border-slate-200 rounded-xl bg-white text-left active:bg-slate-50 transition-colors shadow-sm"
                            >
                        ...
                        <Select value={workResult} onValueChange={(v: 'completed' | 'progress' | 'pending') => setWorkResult(v)}>
                ...
                <div className="space-y-2">
                    ...
                    <Textarea
                        id="obstacles"
                        placeholder="Tuliskan kendala atau hal-hal yang memerlukan perhatian..."
                        className="min-h-[80px] resize-none text-sm border-slate-200 focus:border-blue-400 focus:ring-blue-400/20"
                        value={obstacles}
                    ...
                    <div className="flex gap-3">
                        {MOOD_OPTIONS.map((option) => (
                            <button
                                ...
                                `}
                            >
                                <span className="text-4xl filter drop-shadow-sm transition-transform duration-200" style={{
                                    transform: mood === option.value ? 'scale(1.1)' : 'scale(1)'
                                }}>{option.value}</span>
                                <span className={`text-xs font-bold ${mood === option.value ? 'text-blue-700' : 'text-slate-500'}`}>
                                    {option.label}
                                </span>
                            </button>
                        ))}
                ...
                {/* Helper Info - Only show relevant desktop/mobile tooltip */}
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 mb-4">
                    <p className="text-xs text-slate-500 flex items-start gap-2 leading-relaxed">
                        ...
                        <span>
                            <strong>Tips:</strong> Simpan sebagai Draft jika belum selesai. Manager akan membaca jurnal yang sudah dikirim.
                        </span>
                    </p>
                </div>
            </div>
            ...
            {/* Sticky Actions */}
            <div className={`mt-auto pt-4 border-t border-slate-100 flex flex-col-reverse sm:flex-row gap-3 ${isMobile ? 'sticky bottom-0 bg-white/95 backdrop-blur-sm -mx-1 px-1 py-4 z-10' : ''}`}>
                <Button
                    variant="ghost"
                    onClick={onCancel}
                    disabled={isSubmitting}
                    className="text-slate-500 h-12 sm:w-auto"
                >
                    Batal
                </Button>
                ...
                <Button
                    ...
                    disabled={isSubmitting || !content.trim()}
                    className="gap-2 bg-blue-600 hover:bg-blue-700 text-white flex-1 h-12 text-base font-bold shadow-lg shadow-blue-200 hover:shadow-blue-300 transition-all"
                >
