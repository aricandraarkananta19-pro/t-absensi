    ...
    return (
        <div className="w-full mb-8 space-y-4">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 w-full">
                {/* Status Tabs */}
                <div className="bg-slate-100/80 p-1.5 rounded-2xl w-full md:w-auto overflow-x-auto shadow-inner border border-slate-200/50">
                            ...
                            return (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        status === s
                                            ? "bg-white text-slate-900 shadow-[0_2px_8px_-2px_rgba(0,0,0,0.08)] border border-slate-200/60"
                ...
                <div className="relative w-full md:w-[320px] shrink-0">
                    ...
                    <Input
                        ...
                        className="pl-11 pr-10 bg-white border-slate-200/80 rounded-2xl h-11 font-medium text-sm shadow-[0_2px_8px_-4px_rgba(0,0,0,0.05)] focus:ring-4 focus:ring-blue-500/10 placeholder:text-slate-400 transition-all border outline-none"
                        value={search}
            ...
            <Collapsible open={isAdvancedOpen} onOpenChange={setIsAdvancedOpen} className="w-full">
                <div className="flex items-center justify-between">
                    <CollapsibleTrigger asChild>
                        <Button
                            variant="ghost"
                            size="sm"
                            className="text-slate-500 hover:text-slate-800 hover:bg-slate-100 rounded-xl h-9 px-3 gap-2 font-bold text-xs"
                        >
                ...
                <CollapsibleContent className="CollapsibleContent mt-3">
                    <div className="bg-white border border-slate-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-[20px] p-5 animate-in fade-in slide-in-from-top-2 duration-300">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div className="flex flex-col gap-2">
                                ...
                                <Select value={department} onValueChange={onDepartmentChange}>
                                    <SelectTrigger className="w-full bg-slate-50/50 border-slate-200 rounded-xl h-11 text-sm font-semibold text-slate-700 shadow-sm focus:ring-4 focus:ring-blue-500/10">
                                        <SelectValue placeholder="Semua Departemen" />
                                    </SelectTrigger>
                                    <SelectContent className="rounded-2xl border-slate-200 shadow-xl p-1">
                                        <SelectItem value="all" className="rounded-xl font-medium text-sm my-0.5 focus:bg-slate-100">Semua Departemen</SelectItem>
                                        {['IT', 'HR', 'Finance', 'Marketing', 'Operations'].map((dept) => (
                                            <SelectItem key={dept} value={dept} className="rounded-xl font-medium text-sm my-0.5 focus:bg-slate-100">{dept}</SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                            </div>
                                ...
                                <Popover>
                                    <PopoverTrigger asChild>
                                        <Button
                                            variant="outline"
                                            className={cn(
                                                "w-full justify-start text-left text-sm font-semibold bg-slate-50/50 border-slate-200 rounded-xl h-11 shadow-sm transition-colors",
                                                date ? "border-blue-300 bg-blue-50/50 text-blue-700" : "text-slate-500"
                                            )}
                                        >
                                    ...
                                    <PopoverContent className="w-auto p-4 rounded-[24px] shadow-xl border-slate-200" align="start">
                                        <Calendar
                                            mode="single"
                                            selected={date}
                            ...
                            <div className="flex flex-col gap-2 opacity-50 pointer-events-none hidden md:flex">
                                ...
                                <Select disabled>
                                    <SelectTrigger className="w-full bg-slate-50/50 border-slate-200 rounded-xl h-11 text-sm font-semibold text-slate-400 shadow-sm">
